...
In this section, we present results for various HEP applications. Section~\ref{subsec4.1} demonstrates baseline performance for end-to-end fast simulation, as done with traditional normalizing flows \cite{flashsim}. Section~\ref{subsec4.2} extends the approach to much higher dimensions, showing that flowBDT maintains good performance for hundreds of calorimeter voxels and jet constituents. Section~\ref{subsec4.3} illustrates conditional generation for unfolding, and Section~\ref{subsec4.4} demonstrates Schr{\"o}dinger Bridge refinement for mapping between fast and full simulation distributions.
...
\begin{ruledtabular}
        ...
        \hline
        \textbf{$d_{12}$} & $0.059 \pm 0.02$ & $0.003 \pm 0.001$  \\
        \textbf{$d_2$} & $0.14 \pm 0.03$ & $0.004 \pm 0.001$ \\
        \textbf{$d_{23}$} & $0.006 \pm 0.004$ & $0.095 \pm 0.008$ \\
        \textbf{$\tau_{1}$} & $0.096 \pm 0.02$ & $0.042 \pm 0.005$\\
        \textbf{$\tau_{2}$} & $0.15 \pm 0.03$ & $0.007 \pm 0.002$\\
        \textbf{$\tau_{3}$} & $0.065 \pm 0.02$ & $0.009 \pm 0.002$\\
        \textbf{$mass$} & $0.086 \pm 0.02$ & $0.003 \pm 0.001$ \\
        \textbf{$p^\mathrm{T}$} & $0.051 \pm 0.02$ & $0.009 \pm 0.002$ \\
        \textbf{$\tau_{21}$} & $0.045 \pm 0.02$ & $0.020 \pm 0.003$\\
        \textbf{$ecf_2$} & $0.084 \pm 0.02$ & $0.003 \pm 0.001$ \\
        \textbf{$ecf_3$} & $0.0001 \pm 0.001$ & $0.15 \pm 0.01$\\
        \textbf{$\tau_{32}$} & $0.010 \pm 0.006$ & $0.031 \pm 0.004$\\

       \end{tabular}

     \caption{Evaluation metrics for samples generated from flowBDT. The separation power (triangular discriminator) is calculated from binned histograms. Wasserstein-1 (W1) distance is derived from unbinned distributions. Uncertainties are estimated via bootstrap resampling ($n=100$).}
     \label{tab1}
\end{ruledtabular}
\end{table}
...
The same treatment is applied to the JetNet dataset, where the goal is to regress the raw particle-level four-vector data for gluon-initiated and top jets. The generated relative $p^\mathrm{T}$ distribution shows excellent alignment with the target. A small deficiency is observed in the tails of the relative angular distributions compared to the original data, as illustrated in Figure~\ref{fig7}. This discrepancy highlights the limitations of flowBDT when applied to more structured data, such as point clouds with inherent spatial relationships.
...
We evaluated the generated high-level features for complex top jet topologies using both the triangular discriminator and Wasserstein-1 distance with bootstrap uncertainties, finding strong agreement for most features. We extended the approach to much higher dimensions for the simulation of calorimeter voxels and jet constituents, where the model continues to perform well. We further demonstrated the advantages of conditional generation: for unfolding, the relative improvement in correlation agreement with the target is over 30 times higher when starting from a reconstructed-level prior. Conditional generation can also be applied to calorimeter shower refinement, using approximate showers that encode geometry and material information.

Looking ahead, we plan to extend BUFF to additional tasks such as anomaly detection and quark-gluon tagging. For low-level simulations, we will explore strategies to further enhance efficiency. The code for training, sampling, and evaluation is publicly available at \url{https://github.com/your-repo/BUFF}.
